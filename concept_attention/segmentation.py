...
def encode_image(
    ...
    if isinstance(image, PIL.Image.Image):    
        transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Lambda(lambda x: 2.0 * x - 1.0),    # arun - converting image into a tensor where values lie from [0, 1] and scaling it to [-1, 1]
        ])
        image = transform(image)
    else:    # arun - if not PIL assuming it is already a tensor
        transform = transforms.Compose([
